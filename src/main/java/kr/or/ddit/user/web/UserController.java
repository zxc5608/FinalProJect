...
//		model.addAttribute("userVo", userVo);
	...
	public String Join(UsersVo userVo, String p_no1, String p_no2, String p_no3, String brthday) {
		...
		if(cnt == 1) {
			return "pms_col/login";	//프로젝트생성페이지로
		}else {
			return "pms_col/signup";
		}
...
//			return "pms_col/JYHjoin";
	...
	@RequestMapping("rightmember")
	public String rightmember(Model model, HttpSession session) {
		String prjctno = (String)session.getAttribute("prjctno");
		
		List<UsersVo> users = userService.selectProfile(prjctno);
		List<MemberVo> member = userService.selectMemberPr(prjctno);
